...
public abstract class BaseRepository<T extends BaseEntity<T>> implements BaseRepositoryInterface<T> {
	...
	private int count(String query, UnaryOperator<PreparedStatement> setArgsOperator) {
		...
		try (Connection connection = DatabaseAccessor.getDatabaseConnection()) {
			...
			ResultSet rs = ps.executeQuery();
			System.out.println("My count is "+ rs);
			if (rs.next()) {
				count = rs.getInt(1);
			}
		} catch (SQLException e) {
	...
	//price
	protected BigDecimal priceWhere(WhereContainer where, UnaryOperator<PreparedStatement> setArgsOperator) {
		return price(
			selectQuery(
				(new String[] { PRICE_PROJECTION }),
				null, where, null, INVALID_INDEX, INVALID_INDEX
			),
			setArgsOperator
		);
	...
	private BigDecimal price(String query, UnaryOperator<PreparedStatement> setArgsOperator) {
	...
	private static final String PRICE_PROJECTION = "PRICE(*)";
